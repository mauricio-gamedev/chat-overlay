...
- Mantém o mesmo núcleo realtime validado nas versões anteriores.
- Conexão nativa ao chat público da Kick, sem Chrome e sem WebView.
- Fundo do painel 100% transparente.
- Cor real do nome recebida da identidade da Kick (`username_color` / `color`), com cor estável de fallback por usuário.
- Texto das mensagens também colorido para melhorar leitura durante gameplay.
- Badges desenhados como ícones leves em vez de etiquetas retangulares de texto.
- Emotes da Kick renderizados dentro das mensagens com cache local; em Android 9+ emotes animados podem permanecer animados.
